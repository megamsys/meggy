...
MOAR_HELP
    Meggy::Pug.list_commands 
      ...
      def help_topics
        ...
        # This constant is provided in help_topics.rb which is automatically required/loaded by the pug subcommand loader.
        HELP_TOPICS
      end

      def print_help_topics
        ui.info "Available help topics are: "
        help_topics.collect {|t| t.gsub(/pug-/, '') }.sort.each do |topic|
          ui.msg "  #{topic}"
        end
      end
      ...
      def find_manpages_for_query(query)
        possibilities = help_topics.select do |manpage|
          ::File.fnmatch("pug-#{query}*", manpage) || ::File.fnmatch("#{query}*", manpage)
        end
