...
class Meggy
  class Meg
    ...
    #we load all the files under an application, if its meg then the ruby files under
    ...
    def self.find_subcommands_via_dirglob
      ...
      #  __FILE__ points to ~meggy/lib/meggy/meg.rb
      # Grab the directory of meg command files.
      files = Dir[File.expand_path('../meg/*.rb', __FILE__)]
      #files.each { |x| puts "#{x}"} #
      subcommand_files = {}
      # Under each of the meggy/meg directory, Grab the *.rb files.
      # subcommand_files stores it in a hash
      # eg: {"meggy/meg/identity_list"=>"/home/ram/code/megam/workspace/meggy/lib/meggy/meg/identity_list.rb"}
    ...
    # Print the list of subcommands meg knows about. If +preferred_category+
    # is given, only subcommands in that category are shown
    def self.list_commands(preferred_category=nil)
      load_commands  # load the commands.
      category_desc = preferred_category ? preferred_category + " " : ''
      msg "Available #{category_desc}subcommands: (for details, meg SUB-COMMAND --help)\n\n"
    ...
    # Run meg for the given +args+ (ARGV), adding +options+ to the list of
    # CLI options that the subcommand knows how to handle.
    # ===Arguments
    # args::: usually ARGV
    # options::: A Mixlib::CLI option parser hash. These +options+ are how
    # subcommands know about global meg CLI options
    def self.run(args, options={})
      load_commands # load all subcommands in meggy/meg/*.rb in separated categories.
      subcommand_class = subcommand_class_from(args) # figure out the subcommand class from args.
      subcommand_class.options = options.merge!(subcommand_class.options) # merge both the options in app::meg into this one.
      subcommand_class.load_deps #if there is a deps block inside the subcommand class load them.
      # Time to get started, instantiate the subcommand class. This calls the initialize of Meggy::Meg.
    ...
    def self.subcommand_class_from(args)
      ...
      # see if we got the command as e.g., meg node-list
    ...
    # Returns a subset of the Meggy::Config[:meg] Hash that is relevant to the
    # currently executing meg command. This is used by #configure_meggy to
    # apply settings from meg.rb to the +config+ hash.
    ...
    def locate_config_file
      # Look for $HOME/.megam/meg.rb
      if ENV['HOME']
        user_config_file =  File.expand_path(File.join(ENV['HOME'], '.megam', 'meg.rb'))
      end
    ...
    # settings from config file, via Meggy::Config[:meg]
    ...
    # configure meggy, to startwith locate the config file under .megam/meg.rb
    # Once located, read the meg.rb config file. parse them, and report any ruby syntax errors.
    ...
    # reads all the config from the .megam/meg.rb and stores it inside Meggy::Config
    def read_config_file(file)
      ...
      #ERROR: You have invalid ruby syntax in your config file /home/ram/.megam/meg.rb
      #/home/ram/.megam/meg.rb:9: syntax error, unexpected '='
      #meg[:username] > = "admin"
      #                  ^
      # # /home/ram/.megam/meg.rb
      #  8: meggy_server_url          'http://localhost:6167'
      #  9: meg[:username] > = "admin"
      # 10: meg[:password] = "team4dog"
