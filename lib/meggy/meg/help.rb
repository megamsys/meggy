...
class Meggy
  class Meg
    class Help < Meg
      ...
      banner "meg help [list|TOPIC]"
      def run
        if name_args.empty?
          text.info "Usage: meg SUBCOMMAND (options)"
          text.msg ""
          # This command is atypical, the user is likely not interested in usage of
          # this command, but meg in general. So hack the banner.
          opt_parser.banner = "General Meg Options:"
          text.msg opt_parser.to_s
          text.msg ""
          text.info "For further help:"
          text.info(<<-MOAR_HELP)
  meg SUBCOMMAND --help     show the options for a command
MOAR_HELP
      ...
      def help_topics
        ...
        # This constant is provided in help_topics.rb which is automatically required/loaded by the meg subcommand loader.
        HELP_TOPICS
      end

      def print_help_topics
        ui.info "Available help topics are: "
        help_topics.collect {|t| t.gsub(/meg-/, '') }.sort.each do |topic|
          ui.msg "  #{topic}"
        end
      end
      ...
      def find_manpages_for_query(query)
        possibilities = help_topics.select do |manpage|
          ::File.fnmatch("meg-#{query}*", manpage) || ::File.fnmatch("#{query}*", manpage)
        end
      ...
      def find_manpage_path(topic)
        if ::File.exists?(::File.expand_path("../distro/common/man/man1/#{topic}.1", MEGGY_ROOT))
          # If we've provided the man page in the gem, give that
          return ::File.expand_path("../distro/common/man/man1/#{topic}.1", MEGGY_ROOT)
        else
